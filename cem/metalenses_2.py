...
def plot_density(metalens, quantity, extra = '',
    ...
    plt.rcParams.update({'font.size': 32})
    fig, ax = plt.subplots(1,figsize=(20,18))
    ims = ax.imshow(metalens[quantity],extent=metalens['extent'],
              cmap = metalens['cmap'],
              origin='lower',
...
def plot_simulation_cell(metalens):
    ...
    ax.add_patch(plt.Rectangle((-metalens['sim_cell_width']/2,
            ...
            metalens['s'] + metalens['gap'] + metalens['pml_width'],
            fc=(0,0,1,0.2),
            hatch='/'))
    ax.set_aspect('equal')
    plt.xlabel('x/$\mu$m')
