...
from zialab.instruments.pipython.interfaces.pigateway import PIGateway
from zialab.instruments.pipython import GCSError, gcserror

__signature__ = 0x6b8296d86845fb828ca9c318321c2586
...
class PIUSB(PIGateway):
    ...
    def read(self):
        ...
        received = received.rstrip('\0')  # some controllers return their answer in a size of modulus 2
        return received
...
def getdevinfo(dev, stringid):
    ...
    """
    try:
        return re.sub('[^a-zA-Z0-9-_*. ]', '', usb.util.get_string(dev, stringid).encode('utf-16-le'))
    except ValueError:
