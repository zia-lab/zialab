...
flaskPort = 7778 # The TCP port at which the API will serve
cryoIP = '10.9.93.129' # IP of the PC connected to the cyrostation
cryoPort = 7773 # The port in that PC where one connects to it
...
@app.route('/')
def readme():
    readmeh = '''Welcome to the north pole.'''
    return readmeh

@app.route('/getmontana')
def getem():
    state_of_montana = list(cryostation.get_full_state())
    state_of_montana = list(map(str,state_of_montana))
    state_of_montana = ','.join(state_of_montana)
    return state_of_montana

if __name__ == '__main__':
    cryostation = Montana(ip=cryoIP, port=cryoPort)
    try:
