...
class Monitor():
    ...
    at a given sampling rate. A few convenient
    ...
    def __init__(self,
        ...
        self.timerange = time_range  # in seconds
        ...
        self.itercount = 0
        self.min_y_field_label = tk.Label(master=self.root, text="y_min = ",
                                          bg='black', fg='white', anchor='w')
        self.min_y_field = tk.Entry(master=self.root,
        ...
        self.max_y_field_label = tk.Label(master=self.root, text="y_max = ",
                                          bg='black', fg='white', anchor='w')
        self.max_y_field = tk.Entry(master=self.root,
        ...
        self.info_label = tk.Label(master=self.root, text=self.info,
                                   bg='black', fg='white', borderwidth=2,
                                   relief='ridge')
        self.config_label = tk.Label(master=self.root,
                                     text='% 5.2f Hz' % self.sampling_rate_Hz,
                                     bg='black', fg='white')
    ...
    def init_window(self):
        def animate(i):
            ...
            if self.pause:
                return None
            # when it's filled up, pop the first one
            if len(self.times) >= self.max_records:
                self.times.pop(0)  # let one go
                self.counts.pop(0)  # here, too
            self.counts.append(self.get_counts())
            # compute the actual refresh rate, and try and match it to target
            if len(self.times) > 2:
...
#                print(new_interval)
            ...
            self.times.append((time()-self.start_time)/self.time_unit)
            # update the data
            self.line.set_ydata(self.counts)
            ...
            if self.times[-1]*self.time_unit >= self.timerange:
                xlims = ((self.times[-1] * self.time_unit - self.timerange) /
                         self.time_unit, self.times[-1])
        ...
        if not self.auto_y_range:
            self.ax.set_ylim(self.y_min, self.y_max)
        self.line, = self.ax.plot([], [], 'o-', ms=2, c=self.linecolor)

        self.line2, = self.ax.plot([], [], '--', c='white')
...
#                exit()
        ...
        self.frame.grid(row=0, column=0,
                        columnspan=self.tk_columns, padx=20, pady=(20, 0))
        self.min_y_field_label.grid(sticky='e', row=1,
                                    column=0, pady=self.botton_row_pady)
        self.min_y_field.grid(sticky='w', row=1,
                              column=1, pady=self.botton_row_pady)
        self.max_y_field_label.grid(sticky='e', row=1,
                                    column=2, pady=self.botton_row_pady)
        self.max_y_field.grid(sticky='w', row=1,
                              column=3, pady=self.botton_row_pady)
        self.info_label.grid(row=1,
                             column=4, pady=self.botton_row_pady)
        self.config_label.grid(row=1,
                               column=5, pady=self.botton_row_pady)
...
mon = Monitor(sampling_frequency=10, time_range=time_range)
mon.run()
